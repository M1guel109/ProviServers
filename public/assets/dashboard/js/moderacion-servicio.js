...
function cargarDetalleServicio(id) {
  ...
  fetch(`${BASE_URL}/admin/servicio-detalle?id=${id}`)
    ...
    .then((data) => {
      ...
      setText(
        "modal-proveedor-ubicacion",
        data.proveedor_ubicacion || "Sin ubicación",
      );
...
// Agrega esta función al final o cerca de tus funciones de confirmación
function getPendientesCount() {
    // Busca todas las celdas o elementos que indiquen "PENDIENTE"
    // (Asegúrate de que tus celdas de estado en la tabla tengan alguna clase identificativa, 
    // por ejemplo, si usas un badge: <span class="badge bg-warning">PENDIENTE</span>)
    const pendientes = document.querySelectorAll('.badge-warning'); 
...
function confirmarAprobacion(id) {
    const pendientesCount = getPendientesCount();
  Swal.fire({
    title: "¿Aprobar servicio?",
    text: `El servicio será visible para todos. Quedan ${pendientesCount} publicaciones por moderar.`,
    icon: "question",
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function styleSearchInput(wrapperSelector) {
    ...
    input.style.cssText =
      "width:100%; border:none; background:transparent; outline:none; padding-left:10px;";
  }
