...
function calcularDiasRestantes($fecha_fin)
{
    ...
    return ($hoy > $fin) ? -1 * $diferencia->days : $diferencia->days;
}
?>
<!DOCTYPE html>
<html lang="es">
...
<body>
                            ...
                            <tbody>
                                <?php if (!empty($suscripciones)) : ?>
                                    <?php foreach ($suscripciones as $sub) : ?>
                                        <?php
                                        // Proteger contra fechas NULL
                                        $dias            = (!empty($sub['fecha_fin'])) ? calcularDiasRestantes($sub['fecha_fin']) : -1;
                                        $estado_norm     = strtoupper($sub['estado'] ?? '');
                                        $es_activo       = ($estado_norm === 'ACTIVA' && $dias >= 0);
                                        ?>
                                        <tr>
                                            ...
                                            <td>
                                                <?= !empty($sub['fecha_inicio'])
                                                    ? date('d/m/Y', strtotime($sub['fecha_inicio']))
                                            ...
                                            <td>
                                                ...
                                                <?php elseif ($estado_norm === 'CANCELADA'): ?>
                                                    <span class="badge bg-dark bg-opacity-10 text-dark border border-dark px-3 rounded-pill">
                                                        Cancelada
                                                    </span>
                                                <?php elseif ($dias < 0): ?>
                                                ...
                                                <?php else: ?>
                                                    <span class="badge bg-secondary bg-opacity-10 text-secondary border border-secondary px-3 rounded-pill">
                                                        <?= htmlspecialchars(ucfirst(strtolower($sub['estado'] ?? ''))) ?>
                                                    </span>
                                                <?php endif; ?>
                                            </td>
                                            <td>
                                                <?php if (empty($sub['fecha_fin'])): ?>
                                            ...
                                            <td>
                                                <div class="action-buttons">
                                                    <button type="button"
                                                            ...
                                                            title="Ver detalle"
                                                            onclick="cargarDetalleSuscripcion(<?= (int)$sub['id'] ?>)">
                                                        <i class="bi bi-eye"></i>
                                                    </button>
                                                    ...
                                                    <?php if ($es_activo): ?>
                                                        <button type="button"
                                                                ...
                                                                onclick="confirmarCancelacion(<?= (int)$sub['id'] ?>)">
                                    ...
                                    <?php if (!empty($suscripciones)) : ?>
                                        <?php foreach ($suscripciones as $sub) : ?>
                                            <tr>
                                                ...
                                                <td><?= htmlspecialchars($sub['nombre_proveedor'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($sub['email'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($sub['nombre_plan'] ?? '') ?></td>
                                                <td>$ <?= number_format((float)($sub['costo'] ?? 0), 0, ',', '.') ?></td>
                                                <td><?= htmlspecialchars($sub['fecha_inicio'] ?? '') ?></td>
                                                <td><?= htmlspecialchars($sub['fecha_fin'] ?? '') ?></td>
                                                <td><?= htmlspecialchars(ucfirst(strtolower($sub['estado'] ?? ''))) ?></td>
                                            </tr>
                                        <?php endforeach; ?>
                                    <?php endif; ?>
                                </tbody>
                            ...
                            <div class="bg-light p-4 border-bottom">
                                <div class="d-flex align-items-center">
                                    ...
                                    <div class="ms-auto text-end">
                                        <span id="modal-sub-estado-badge"
                                              class="badge rounded-pill px-3 py-2 fs-6"></span>
                                    </div>
                                </div>
                            </div>
                                        ...
                                        <div class="card h-100 border-0 shadow-sm">
                                            <div class="card-body">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <span id="modal-sub-plan"
                                                          class="fs-5 fw-bold text-dark"></span>
                                                </div>
                                                <div class="row">
                                                    <div class="col-6">
                                                        <small class="text-muted d-block text-uppercase"
                                                               style="font-size:0.75rem;">Costo</small>
                                                        <span id="modal-sub-costo"
                                                              class="fw-bold text-success"></span>
                                                    </div>
                                                    <div class="col-6">
                                                        <small class="text-muted d-block text-uppercase"
                                                               style="font-size:0.75rem;">ID Referencia</small>
                                                        <span id="modal-sub-id"
                                                              class="font-monospace text-secondary"></span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
    ...
    <!-- ✅ SweetAlert primero, sin apexcharts ni dashboard.js -->
