...
<body>
        ...
        <tr>
            <td>
                <div class="stat-card">
                    ...
                    <span class="stat-label">Total de valoraciones</span>
                </div>
            </td>
            <td>
                <div class="stat-card">
                    <span class="stat-number"><?= number_format($prom, 1) ?></span>
                    <span class="stat-label">Promedio global (/ 5)</span>
                </div>
            </td>
            <td>
                <div class="stat-card">
                    <span class="stat-number"><?= $dist[5] + $dist[4] ?></span>
                    <span class="stat-label">Valoraciones positivas (4-5 ★)</span>
                </div>
            </td>
            <td>
                <div class="stat-card">
                    <span class="stat-number"><?= $dist[1] + $dist[2] ?></span>
                    <span class="stat-label">Valoraciones bajas (1-2 ★)</span>
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="dist-table">
        <?php foreach ([5, 4, 3, 2, 1] as $estrella): ?>
            ...
            ?>
            <tr>
                <td class="star-label"><?= str_repeat('★', $estrella) ?><?= str_repeat('☆', 5 - $estrella) ?></td>
                <td style="width:60%;">
                    <div class="bar-bg">
        ...
        <table class="table-reporte">
            ...
            <tbody>
                <?php foreach ($reporte['topProveedores'] as $i => $prov): ?>
                    <?php
                        $promProv = (float)$prov['promedio'];
                        if ($promProv >= 4.0) {
                            $badge = '<span class="badge-alto">Excelente</span>';
                        } elseif ($promProv >= 3.0) {
                            $badge = '<span class="badge-medio">Regular</span>';
                        } else {
                            $badge = '<span class="badge-bajo">Bajo</span>';
                        }
                    ?>
                    <tr>
                        <td class="text-center"><?= $i + 1 ?></td>
                        <td><?= htmlspecialchars($prov['proveedor']) ?></td>
                        <td class="text-center stars"><?= number_format($promProv, 2) ?> ★</td>
    ...
    <div class="footer">
        Proviservers — Reporte de Calificaciones — <?= date('d/m/Y H:i') ?>
    </div>
